...
One record per booking attempt. Created when guest initiates checkout (Stripe PaymentIntent created). Status updated via Stripe webhook.
...
| GSI2PK | `STRIPE#{paymentIntentId}` | `STRIPE#pi_3P...` |
...
- Resolve booking from Stripe webhook → `GSI2: GSI2PK=STRIPE#pi_3P...`
...
**Access patterns:**
- Get all guidebook sections for property (guest view) → `PK=PROPERTY#kentucky, SK begins_with GUIDEBOOK#SECTION#` (returns in order)
...
| GSI2 | `STRIPE#{paymentIntentId}` | `BOOKING#{id}` | Resolve booking from Stripe webhook |
...
| Get guidebook for guest | Query | `PK=PROPERTY#kentucky, SK begins_with GUIDEBOOK#SECTION#` |
...
| Resolve booking from Stripe | GSI2 Query | `GSI2PK=STRIPE#pi_3P...` |
