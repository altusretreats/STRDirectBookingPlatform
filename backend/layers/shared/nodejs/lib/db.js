...
const { DynamoDBDocumentClient, GetCommand, QueryCommand, PutCommand, UpdateCommand, DeleteCommand, TransactWriteCommand } = require('@aws-sdk/lib-dynamodb');
...
module.exports = {
  ...
  transactWrite: (actions) => ddb.send(new TransactWriteCommand({
    TransactItems: actions.map((action) => {
      const operation = Object.keys(action)[0];
      return { [operation]: { TableName: TABLE, ...action[operation] } };
    }),
  })),
};
