...
- **Guidebook place items** (`type: 'place'`) store Google Places v2 data under `item.place`. `item.place.mapIcon` is an optional admin-selected homepage marker glyph (`food`, `arch`, `climber`, `carabiner`, `trail`, `paddle`, `offroad`, `shopping`, `services`, or `pin`); blank values use the place-category default. `aiContext` is hidden from guests and available only through the curated agent feed; `hostNotes` remains admin-only.
