...
- **Add Item is a focused subview, not an inline form (2026-08-14).** The Section editor's item list has a **+ Add Item** button instead of an always-visible add form. Clicking it swaps the same modal's body into an "Add an item" (or "Add a place" for Local Recommendations) subview — no second/stacked modal. General sections choose from the existing supported types via cards (Text, Guide, Image, Video, Map Link, Web Link); recommendations sections keep the Google Place lookup/preview workflow as the only addition type, unchanged. "Add to Section" appends to the local draft and returns to the section view with a "{item} was added to this draft. Save the section to keep this change." notice; nothing is persisted until **Save Section**. Back to section / Cancel / Escape all discard only the unfinished item and return to the section view without touching the rest of the draft or closing the modal.
