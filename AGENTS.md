...
- **`staytheoverhang.com` (bare root)** → GoDaddy domain forwarding issues a 301 to `https://www.staytheoverhang.com` (verified 2026-08-14). It is not served directly by CloudFront, but the forward means visitors and search engines always land on the real `www` site.
- The distribution has aliases for both `staytheoverhang.com` and `www.staytheoverhang.com`, so a direct CloudFront ALIAS at the apex remains possible later, but is not required — the GoDaddy forward already resolves the "why don't I see my change at bare root" confusion.
...
- The durable editorial source for recommendation descriptions and AI context is [`docs/guest-guide-knowledge-base.md`](docs/guest-guide-knowledge-base.md). It is reference material, not a deployment source: live content is still saved through the property-scoped Admin Guidebook.
...
- The guest guide header uses the same transparent Overhang logo asset as the property site (`frontend/property-site/img/logo-the-overhang.png`); the former generic mountain badge has been removed.
- Sections are grouped client-side into Welcome in / Home essentials / Food / Explore / Checking out, so adding a property remains a data operation. The internal journey key remains `arriving`, but its guest-facing language assumes the guide is accessed inside the residence rather than before arrival. Restaurant place items are projected into the dedicated Food journey while every other place remains in Explore, including when both types share one admin-managed recommendations section. Existing text, image, video, map, link, copyable code, search, and Google Place item types remain supported.
- Guidebook sections and individual items support reusable `audiences` tags: `hikers`, `climbers`, `offroaders`, `golfers`, `families`, and `nightlife`. Explore renders conversational filter chips only for audiences represented by published data. A section tag includes the whole section; item tags project matches from mixed recommendation sections.
- `guide` items provide safe longer-form editorial content using limited Markdown paragraphs, bullets, and HTTPS links. They are intended for packing lists and audience-specific tips. Affiliate links are allowed only with a clear commission disclosure immediately before the recommendations; Amazon Associates additionally requires its associate-identification statement clearly on the guide and the guide site registered in the account. Do not embed storefronts, unverified prices, or arbitrary HTML.
- Guest-facing navigation and place-fallback icons use one restrained deep-blue outline SVG sprite (`frontend/property-site/guidebook/index.html`).
- **Section icons — Material Symbols (2026-08-14).** Admins pick a section icon from a searchable Material Symbols Outlined (weight 400, no fill) picker in `GuidebookEditor.jsx`, backed by a self-hosted, offline catalog of ~3,899 verified icon names (`frontend/admin-spa/src/data/materialSymbolsCatalog.js`, generated from the `material-symbols` npm package's `index.d.ts`). No new DB field: the existing `section.icon` string stores `material-symbols:<name>` (e.g. `material-symbols:hot_tub`), validated by `parseSectionIcon()`. Legacy emoji values and blank/invalid values are untouched and remain fully editable (a secondary "Emoji (legacy)" tab stays in the picker); the guest Guidebook (`guidebook.js`'s `sectionIconRef()`) renders a valid `material-symbols:*` value directly and falls through to the pre-existing title/journey/audience heuristic (`sectionIconName()` → the SVG sprite) for everything else. No destructive migration was run — old sections keep their emoji until a human re-saves them from the new picker. The font is self-hosted (not fonts.googleapis.com) in both apps: bundled via Vite in `frontend/admin-spa` (`import 'material-symbols/outlined.css'` in `main.jsx`) and committed as static files in `frontend/property-site/guidebook/fonts/material-symbols-outlined.woff2` + `css/material-symbols.css`. Both apps gate `.material-symbols-outlined` at `opacity:0` until `document.fonts.ready` adds a `.symbols-ready` class, so ligature names (e.g. "coffee_maker") never flash as literal text. The specialized homepage map/attraction marker system (`place.mapIcon`, `frontend/property-site/js/app.js`, `img/map-icons/*.svg`) is unrelated and unchanged.
...
- Guidebook sections support an admin-managed `important` flag. Important sections receive a prominent guest-facing label and accent treatment, open by default, and are prioritized in Quick essentials without changing their journey grouping or section icon.
- The admin section editor treats added content and Google Places as draft changes until **Save Section** is clicked. Adding an item now shows an explicit save reminder; URL-based map/link items accept an omitted label and receive a sensible default instead of failing silently.
- **Add and Edit Item are both focused subviews, not inline forms (2026-08-14).** The Section editor's item list has a **+ Add Item** button and a per-item Edit (✏️) button instead of an always-visible add form or in-row expanding editor. Both swap the same modal's body into a focused subview — no second/stacked modal. Add opens "Add an item" (or "Add a place" for Local Recommendations); Edit opens "Edit item" / "Edit place" pre-filled with that item's current fields (including host notes, which Add doesn't collect). General sections choose from the existing supported types via cards (Text, Guide, Image, Video, Map Link, Web Link) when adding; recommendations sections keep the Google Place lookup/preview workflow as the only addition type; editing a place shows its full field set directly (name, category, map marker, audiences, Our Pick, website visibility, description, AI context, host notes) rather than re-running the lookup. "Add to Section" / "Save Changes" write into the local draft and return to the section view with a "{item} was added/updated in this draft. Save the section to keep this change." notice; nothing is persisted until **Save Section**. Back to section / Cancel / Escape all discard only the unsaved add-or-edit and return to the section view without touching the rest of the draft or closing the modal.
...
- Guidebook section icons unified on self-hosted Material Symbols Outlined (2026-08-14, deployed to both admin.altusretreats.net and www.staytheoverhang.com/guidebook/): see the Guidebook doc block above for the full `material-symbols:<name>` format, picker location, and legacy-emoji fallback behavior. No DynamoDB migration was run.
- Fixed the property-page nav's Overview link (2026-08-14): it now targets the photo gallery (`#frame-photo-grid`) instead of the property-story section, and clicking Amenities/Reviews/Location now uses native `scrollIntoView` (respecting each section's CSS `scroll-margin-top`) rather than a hand-computed pixel offset from the nav's height, which broke whenever the nav's height changed mid-scroll (tall landing style → compact scrolled style). The Overview link still needs a manual `OVERVIEW_NAV_CLEARANCE` constant in `js/app.js` (currently `105`, tuned by eye) because the gallery's authored `scroll-margin-top` (88px) stacks additively with the page's `scroll-padding-top` (140px) per spec, landing far short of the compact nav's real height — re-tune that constant directly if the nav is redesigned.
...
- A branded vector Google property map was added 2026-08-13 with a custom property marker, lazy loading, and iframe fallback. Its Map ID must remain associated with the published `Altus Retreats — Property Map` cloud style, which hides all third-party POI labels.
...
- The homepage Location section is now a guidebook-powered local explorer: it retains the branded POI-free Google basemap and logo property marker, adds categorized custom attraction markers, filter chips, rating/distance/drive-time cards, and direct Directions links. The map remains part of the booking page flow instead of becoming a separate page; published guidebook place items are its single source of truth.
- Food discovery, host picks, and recommendation preview controls were deployed 2026-08-13 across the admin, Guidebook, and property site. Restaurant place items now receive their own Food journey and **Eat nearby** property-page group, non-restaurant places remain under Explore / **Things to do nearby**, `item.featured` powers the admin **Our Pick** toggle and guest-facing emphasis, and `item.websiteVisible` lets the complete Guidebook hold more recommendations than the curated property-page preview. Hidden website recommendations trigger an open-ended “And there’s much more” teaser without a finite count.
- Audience-personalized Explore filters and formatted editorial guides were added 2026-08-13. Admins can target whole sections or individual recommendations to Hikers, Climbers, Off-roaders, Golfers, Families, and Nightlife; only populated filters appear to guests. A private hiking packing-list draft demonstrates the `guide` format and disclosure treatment for commissionable product links.
...
- The Overhang full logo is live at `frontend/property-site/img/logo-the-overhang.png`, wired into `index.html`'s nav (`.nav__logo-img`) and the property-title lockup. Its compact hanging-climber artwork is stored separately at `frontend/property-site/img/logo-the-overhang-map-icon.png` and selected through the property-specific `ALTUS_MAPS_CONFIG.markerLogoUrl`, so the Google Map marker can use compact artwork without changing the full site logo. The property map marker is a compact rounded rectangle rather than a capsule, and its pointer is centered beneath the full logo/name lockup so it aligns with the geographic anchor. The property-title treatment removes rounding and renders the transparent mark in black.
- Bare-root `staytheoverhang.com` already 301-forwards to `www.staytheoverhang.com` via GoDaddy domain forwarding (verified 2026-08-14). `altusretreats.net` bare-root does not yet forward to its `www` — it's still on GoDaddy's default parking redirect and needs the same forward configured.
...
- Hospitable widget installed on index.html; `data-site-uuid` may need updating once Direct channel fully configured
...
- `altusretreats.net` bare-root needs GoDaddy domain forwarding to `www.altusretreats.net` configured (currently falls through to GoDaddy's default parking redirect instead)
