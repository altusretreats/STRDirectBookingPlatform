...
- **Homepage place marker icons** — Admin-selectable place glyphs are bundled as local SVG assets under `frontend/property-site/img/map-icons/`: restaurant, coffee, bar, climbing, hiking, trailhead, bridge, kayak, off-road, gondola, zipline, shopping, groceries, gas, scenic/photo, and general location. Existing stored icon keys remain backward-compatible, and the sandstone arch and carabiner remain available. The Overhang's branded property marker is separate and unchanged.
